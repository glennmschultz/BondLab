...
are viewed as bundle of zero coupon cashsflows which are discounted along the
...
Bond Lab® is designed to be modular. All classes are exposed to the user thereby allowing the user to create new superclasses from existing classes or extend the classes as the user sees fit.  Thus, the user is free to combine these classes into a superclass to fit her anlytics needs or extend classes to incorporate additional analytic information or methods.  The Bond Lab® classes are outlined below:
...
### CouponTypes
    The CouponTypes class represents the bond coupon passed to function.  Coupon
    is passed to function in numeric decimal form (5.5).  The CouponTypes class 
    convert the numeric decimal value to a basis 0.55 and character string 
...
### BondScenario
    ...
    It is a superclass of BondReturn, BondCashFlows, BondTermStructure, and 
...
repeatedly.  The use can create her own function to encapsulate the anatomy of 
a Bond Lab®
