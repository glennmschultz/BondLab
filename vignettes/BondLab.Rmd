...
MortgageCashFlow depends on the objects MBSDetail, PrepaymentAssumption, etc.  To examine an example call using MortgageCashFlow type getMethod(MBSAnalytics).  The MortgageCashFlow function creates the cashflow of passthrough security.  It could be used to amortize an unsecuritized mortgage note by changing the MBS security delay to 0. However,
it is recommend that the user create a function for mortgage note amortization schedule with the mortgage amortization
functions provided
...
```
1. bond.id is a character string refrencing a MBS cusip object either a passthrough security or REMIC.
1. original.bal is a numeric value and the original principal amount of the MBS traded.
1. settlement.date is a character string the settlement date entered mm-dd-yyyy
1. price is a numeric value the price of the MBS passthrough
1. PrepaymentAssumption is a character string referencing the PrepaymentAssumption object.

All of the above objects are required inputs to MortgageCashflow a call to MortgageCashFlow is as follows:
```{r, echo=TRUE}
#First, call the MBS passthrough object
MBS <- MBS(MBS.id = "bondlabMBS4")

#Second, call the yield curve for the corresponding settlement date
Rates <- Rates(trade.date = "01-10-2013")

#Third, call TermStructure and MortgageRate.  TermStructure will fit the
#term strucutre using the package Termstrc and MtgRate will propogate the 
#forward mortgage rate based on the forward two- and ten-year passed from 
#the object TermStructure
TermStructure <- TermStructure(rates.data = Rates, method = "ns")
MortgageRate <- MtgRate()

#Fourth, ModelTune calls the model tuning parameter.  The default model tuning
#parameters are associated with MBSDetails object slot Model abd Burnout out is 
#extracted from the MBS object to provide an initial burnout value for the prepayment model
ModelTune <- ModelTune(bond.id = MBS)
Burnout <- MBS@Burnout

#Fifth, PrepaymentAssumption is used to assign a prepayment assumption to MortgageCashFlow
#valid PrepaymentAssumptions are "MODEL", "PPC", "CPR"

Prepay <-PrepaymentAssumption(bond.id = MBS, 
...
#Finally, call MortgageCashFlow.
MortgageCashFlow <- MortgageCashFlow(bond.id = MBS, original.bal = 1000000, 
                 settlement.date = "01-13-2013", 
                 price = 105.75, 
                 PrepaymentAssumption = Prepay)

#The first 24 months of cashflow are
MortgageCashFlow@TotalCashFlow[1:24]
```
...
The basic input is the mortgage loan.  In the agency MBS sector the most basic structure is the pass-through seccurity. A MBS pass-through security represents a pool of mortgage loans and, as its name implies, passes through principal and interest payments to the investor.  The structuring vignette takes the reader through the steps required structure a REMIC.  Waterfally payment logic is scripted seperately.  The ready can examine the waterfall scripts to undestand the basic structure of a waterfall script.

## Valuation

The parametric measures which define the value of an MBS are:

1. Yield
1. Nominal spread
1. Zero volatility spread
1. Option adjusted spread
1. Horizon expected total return

Bond Lab can calcualte each.  Horizon totol return analysis assumes a constant zero volatiltiy spread and a 12 month horizon.
